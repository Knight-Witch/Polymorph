...
## PFC-2026-09-12-027 — Fix console flashes and unify interactive Crop/Fit framing

- Target files: `src/polymorph/probe.py`, `models.py`, `geometry.py`, `filters.py`, `ui/preview.py`, `ui/main_window.py`, `ui/adaptive_main_window.py`, `ui/styles.py`, packaged smoke coverage, framing/probe tests, version metadata, status/architecture/UX/history docs, and required tracking files.
- Relevant history checked before editing: `PROJECT_CONTRACT.md`, `MASTER.md`, `PRE_FLIGHT_Check.md`, `CHANGELOG.md`, `docs/ARCHITECTURE.md`, `docs/UX_SPEC.md`, `HISTORY/REFERENCE_GIF_CONVERTER.md`, `HISTORY/DIAGNOSTICS/GIF_ADAPTIVE_MOTION_2026-09-10.md`, current converter/adaptive converter/probe/geometry/filter/preview/main-window/style/smoke paths, geometry/probe tests, and the Windows build workflow/toolchain gate.
- Human adaptive validation incorporated before touching framing: a confirmed dev.15 Viper Favor-resolution run selected stride 2 and produced 2048x2048 / 82,147,387 bytes / 188 original-source frames over exactly 15.0 s; the user reported the ~12.53 FPS motion looked consistent. Two additional kitbash/decal-heavy 500-frame workloads, including a 3072x3072 source, also selected stride 2, gained ~38% linear resolution, and produced uniform 80 ms output frames. The adaptive selection policy is therefore left untouched.
- Corrected adaptive-history interpretation: the previous Preserve-baseline Viper files remain valid files, but the UI's missing explicit checked-radio indicator made it impossible to rely on the user's visual reading of which priority was selected. A dev.15 screenshot showed Preserve motion was actually checked while Favor resolution was believed selected. Dev.16 fixes the checked-state presentation instead of changing the now-validated algorithm.
- Console-flash diagnosis: FFmpeg/gifski conversion `Popen` calls already use Windows `CREATE_NO_WINDOW`; shared ffprobe in `probe.py` did not. That probe runs both when media is loaded and during post-encode integrity checks, matching the exact reported focus-stealing moments. Recommended action: apply `CREATE_NO_WINDOW` only to that shared ffprobe launch and retain captured stdout/stderr.
- Framing diagnosis: the existing encoder's zoom-1 Crop/Pad filter ordering was conceptually aspect-safe, but preview and encoder used independent framing calculations, and the preview visibly appeared to stretch/distort Crop/Fit. Recommended action: make one shared content-placement model authoritative for both preview and filters, preserving source aspect by cover+clip for Crop and contain+pad for Fit.
- Manual composition requirement: add `FramingSettings.zoom`, expose 100%-400% zoom for Crop/Fit, support mouse-wheel zoom in the preview, keep existing drag positioning, and make Center reset position without silently resetting the user's zoom. Zoom affects composition inside the framing canvas and must not increase the native output-resolution ceiling.
- UI correctness: add explicit visible checked/unchecked radio indicators so selected state can never disappear; this is a correctness fix, not the deferred final visual skin.
- Connected safeguards: final output scale occurs after Crop/Fit composition; fixed-resolution linking still follows the framed canvas ratio; no-upscale sizing remains based on the native framing canvas; Fit background remains user-selectable; Preserve motion, Favor-resolution cadence/thresholds, MP4 settings, GIF quality/tool versions, updater, queue order, and decimal-MB behavior are unchanged.
- Test plan: pure geometry/filter tests must assert square source content remains square inside 16:9 Crop/Fit at normal and zoomed placements; probe tests must assert no-window creation flags; packaged smoke must gate visible radio selection, zoom settings propagation, and aspect-safe Crop/Fit preview. Full Windows CI must then run the unchanged bundled toolchain/adaptive/reference gates before packaging.
- Versioning: increment development tester from `0.1.0-dev.15` to `0.1.0-dev.16`.
- Human dev.16 acceptance: confirm no console flash on load/encoding/optimization; selected radios are obvious; Crop clips instead of stretching; Fit pads instead of stretching; drag/zoom feels correct; and an exported framed result matches the preview composition.

## PFC-2026-09-11-026 — Diagnose dev.13 adaptive fallback without changing selection policy

- Target files: new `src/polymorph/diagnostic_adaptive_converter.py`, `src/polymorph/ui/adaptive_main_window.py`, new `tests/test_adaptive_diagnostics.py`, new `build/verify_adaptive_converter.py`, Windows dev workflow, version metadata, adaptive architecture/UX/build/history/status docs, and required tracking files.
- Relevant history checked: `PROJECT_CONTRACT.md`, `MASTER.md`, `PRE_FLIGHT_Check.md`, `CHANGELOG.md`, `docs/ARCHITECTURE.md`, `docs/UX_SPEC.md`, `HISTORY/REFERENCE_GIF_CONVERTER.md`, `HISTORY/DIAGNOSTICS/GIF_ADAPTIVE_MOTION_2026-09-10.md`, current `converter.py`, `adaptive_converter.py`, `motion_planner.py`, `gif_timing.py`, `integrity.py`, adaptive UI, Windows workflow/toolchain verification, dev.14 UI validation, and the user-supplied dev.13 Viper output.
- Human validation incorporated: dev.14 first-launch sizing now renders correctly and hover tooltips work well. The dev.13 Viper GIF is exactly the same Preserve-motion baseline again: 1552x1552, 93,630,962 bytes, 375 frames at 40 ms over 15.0 s, SHA-256 `dbfd1be7211b801f3a3a8d0ffaaf1058a1974f6b27141ef5f3be5ce55452e5af`.
- Diagnosis: the final dev.13 file proves no stride candidate survived, but it does not distinguish a measured-gain rejection, a completed-fit gain rejection, or a caught adaptive encode/integrity failure. Changing cadence/thresholds again without that evidence would be another blind iteration.
- Recommended action: leave `AdaptiveConverter` selection policy and all encoder settings untouched; wrap only the development Favor-resolution path with structured decision tracing, write a compact JSON sidecar beside the output, and add a deterministic real-toolchain CI integration test that must prove the actual adaptive orchestration can select a lower-frame/larger result when measured byte economics genuinely support the existing >=8% gain requirement.
- Connected modules reviewed: Preserve-motion delegation, adaptive stride state, GIF smart-fit search, final-delay patching, integrity checks, UI worker/result reporting, packaged-app smoke, pinned FFmpeg 9.0.1/gifski 1.32.0 workflow, installer/version metadata, and dev.14 UI geometry/tooltips.
- Conflict risks: diagnostic wrapper accidentally altering decisions; diagnostic file I/O affecting an otherwise-successful conversion; synthetic CI fixture failing to create a sufficiently discriminating byte-cost gap; sidecar clutter leaking into a later stable release; process-level integration test adding build time.
- Safeguards: tracing subclasses the existing adaptive converter without changing its planning/encoding methods; sidecar write failure is non-fatal; Preserve-motion/MP4 do not emit sidecars; dev.14 layout/tooltips remain unchanged; the integration fixture measures its own full/stride byte ratio before asserting selection.
- Versioning: increment development tester from `0.1.0-dev.14` to `0.1.0-dev.15`.
- Unchanged: actual `AdaptiveConverter` candidate order/8% thresholds/2048px soft target, Preserve-motion GIF, MP4, framing, updater, dev.14 presentation, 99 MB decimal ceiling, gifski/FFmpeg dependencies, quality 100, and public release state.

## PFC-2026-09-11-025 — Fix first-launch UI compression and add hover tooltips

- Target files: `src/polymorph/ui/adaptive_main_window.py`, `src/polymorph/ui/styles.py`, `src/polymorph/smoke_test.py`, development version metadata, `docs/UX_SPEC.md`, `MASTER.md`, `PRE_FLIGHT_Check.md`, and `CHANGELOG.md`.
- Relevant history checked: `PROJECT_CONTRACT.md`, `MASTER.md`, `PRE_FLIGHT_Check.md`, `CHANGELOG.md`, `docs/UX_SPEC.md`, current `main_window.py`, `adaptive_main_window.py`, `styles.py`, packaged smoke test, dev.13 version metadata, and the user-supplied screenshots showing the default-height control rail vertically compressed until the window is made taller.
- Diagnosis: the base window still opened at 1080x720, a size chosen before the additional GIF-priority section existed. The adaptive section increased the right rail's vertical content enough that Qt compressed radio/input rows at first launch, visibly clipping field values and labels. The same controls render normally once the user manually increases window height.
- Requested presentation change: make the default UI open at a comfortable height and add concise hover tooltips so controls can remain visually restrained without permanent explanatory copy.
- Connected modules reviewed: base main-window geometry, adaptive section insertion, shared stylesheet, primary/disabled control states, footer tooltips, packaged application smoke path, version/installer metadata. Conversion engine, adaptive encoder, optimizer, preview framing behavior, updater, and dependency/toolchain code are not changed.
- Conflict risks: making the minimum window too tall for smaller displays; over-explaining controls or cluttering the visual design; accidentally touching conversion behavior while a dev.13 Viper encode is still under human test.
- Recommended action: open the adaptive UI at 1080x800, retain a smaller 900x700 resize floor, slightly tighten the right-rail layout spacing, enforce minimum visual heights for section/radio/input controls, improve disabled-state readability, and add hover tooltips to the queue/preview/output/sizing/GIF-priority/framing/output-folder/conversion controls. Extend the packaged smoke test to gate the default geometry and presence of primary tooltips.
- Versioning: increment the UI tester from `0.1.0-dev.13` to `0.1.0-dev.14` so the layout build is unambiguous.
- Unchanged: dev.13 exact source-frame-decimation conversion behavior, Preserve-motion GIF, MP4, framing math/drag semantics, updater, 99 MB decimal ceiling, gifski/FFmpeg toolchain, visual palette, footer destinations, and public release state.
...
- Confirmed real-source evidence: Viper source is 2048x2048, 375 frames, 25 FPS, 15.0 s; OG output is 300 frames over the same duration. Ordinary 25 -> 20 FPS frame selection showed a repeating motion-change spike `[0.6891, 1.1577, 0.6898, 0.6899]`, while motion-compensated interpolation produced near-even phase energy `[0.7742, 0.7898, 0.7902, 0.7755]` and exactly 300 intended frames with end lookahead + trimming.
...
- Confirmed root cause of the larger standalone `1756x1756` result: the standalone wrote source FPS into Y4M with FFmpeg `-r <source fps>` but omitted gifski `--fps`; gifski defaults Y4M/video input to 20 FPS and its Y4M decoder skips frames when required to meet that target.
