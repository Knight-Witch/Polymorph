...
4. Runs the toolchain smoke test against animated WebP, Preserve-motion GIF, adaptive motion-interpolated GIF, MP4, framing filters, exact full-frame retention, and exact planned reduced-frame output. The adaptive smoke path specifically requires bundled FFmpeg `minterpolate` support.
...
The reference timing diagnostic is build-only. Favor resolution itself is included in dev.9 for explicit human testing and remains experimental until full-resolution HeroForge interpolation quality is validated.
