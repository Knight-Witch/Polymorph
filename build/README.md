...
The reference timing diagnostic is build-only. Favor resolution is included in development builds for explicit human testing. Dev.10 selects reduced FPS from real encoded sample cost rather than frame-count prediction and falls back to Preserve motion when the finished lower-FPS result does not earn a meaningful spatial gain.
