...
4. Runs the toolchain smoke test against animated WebP, Preserve-motion GIF, adaptive uniformly resampled GIF, MP4, framing filters, exact full-frame retention, and exact planned reduced-frame output. The adaptive smoke path specifically requires bundled FFmpeg `minterpolate` blend support at the current 12.5 FPS / 80 ms deepest clean cadence.
...
The reference timing diagnostic is build-only. Favor resolution is included in development builds for explicit human testing. Dev.12 keeps real encoded sample cost authoritative but changes the reduced-FPS synthesis method from optical-flow motion compensation to lower-complexity exact-timestamp temporal blending; Preserve motion remains unchanged.
