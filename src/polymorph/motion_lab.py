...
class MotionLab(QMainWindow):
    def __init__(self) -> None:
        ...
        subtitle = QLabel("ELDER FUTHARK / TRANSMUTATION STUDY")
        ...
        self.variant.currentIndexChanged.connect(lambda value: setattr(self.loader, "variant", value))
        ...
        self._add_slider(panel_layout, "Innermost rune ring — CCW", 10, 220, 100, self.loader, "inner_rune_speed")
        self._add_slider(panel_layout, "Partial-window runes — CCW", 10, 220, 100, self.loader, "partial_speed")
        ...
        trace_slider = self._add_slider(panel_layout, "Comet tracer speed", 10, 260, 100, self.loader, "trace_speed")
        trace_slider.valueChanged.connect(lambda value: setattr(self.button, "trace_speed", value / 100.0))
        ...
        self.emblem_status = QLabel(
            "Emblem: bundled Knight Witch SVG"
            if self.loader.emblem
            else "Emblem: bundled SVG unavailable"
        )
        self.emblem_status.setObjectName("Note")
        self.emblem_status.setWordWrap(True)
        panel_layout.addWidget(self.emblem_status)

        panel_layout.addWidget(QLabel("BUTTON STATE"))
        states = QHBoxLayout()
        ...
        note = QLabel(
            "Dense Runes is the current review baseline. The outer progress rings are white; "
            "geometry is red; normal Elder Futhark is gold; designated/large runes are white. "
            "Partial rune frames stay fixed while full rune circles rotate behind opaque windows."
        )
    ...
    def _add_slider(
        ...
        slider.valueChanged.connect(
            lambda raw, obj=target, attr=attribute: setattr(obj, attr, raw / 100.0)
        )
    ...
    def choose_emblem(self) -> None:
        ...
        if self.loader.load_emblem(path):
            ...
            self.emblem_status.setText(
                "Emblem override failed to load; bundled emblem retained only if reloaded on restart."
            )
